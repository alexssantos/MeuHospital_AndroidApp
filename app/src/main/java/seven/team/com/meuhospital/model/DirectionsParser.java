...
/**
 * Created by NeggoRafa on 09/10/2018.
 */

public class DirectionsParser {
    /**
     * Retorna uma lista de listas contendo latitude e longitude de um JSONObject
     */
    public List<List<HashMap<String, String>>> parse(JSONObject jObject) {
        ...
        try {
            ...
            // Loop para todas as rotas
            for (int i = 0; i < jRoutes.length(); i++) {
                jLegs = ((JSONObject) jRoutes.get(i)).getJSONArray("legs");
                List path = new ArrayList<HashMap<String, String>>();

                // Loop para todas as pernas
                for (int j = 0; j < jLegs.length(); j++) {
                    jSteps = ((JSONObject) jLegs.get(j)).getJSONArray("steps");

                    // Loop para todas as etapas
    ...
    /**
     * Método para decodificar polilinha
